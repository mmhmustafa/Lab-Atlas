...
# site=mumbai
#
# Perimeter firewall for mumbai: eth1 is the "outside" leg toward
# mumbai-edge (the inter-city WAN), eth2 is the "inside" leg toward
# mumbai-core (this site's internal network). Not a routing protocol
...
# or the fifth.
ip addr replace 10.90.0.2/30 dev eth1
ip link set eth1 up
ip addr replace 10.90.0.5/30 dev eth2
ip link set eth2 up

ip route replace default via 10.90.0.1 dev eth1
ip route replace 172.30.0.0/22 via 10.90.0.6 dev eth2
ip route replace 10.251.0.0/24 via 10.90.0.6 dev eth2
...
# unrestricted. Only mumbai-core and mumbai-server are reachable from
# other cities (infrastructure + the one "published" node); the access
# tier (mumbai-access1/mumbai-access2) is deliberately internal-only -
...
iptables -A FORWARD -i eth1 -o eth2 -p icmp -d 10.251.0.2 -j ACCEPT
iptables -A FORWARD -i eth1 -o eth2 -p icmp -d 10.251.0.5 -j ACCEPT
iptables -A FORWARD -i eth1 -o eth2 -p tcp --dport 22 -d 10.251.0.5 -j ACCEPT
iptables -A FORWARD -i eth1 -o eth2 -j LOG --log-prefix "ATLASLAB-FW-MUMBAI-DROP: " --log-level 4
